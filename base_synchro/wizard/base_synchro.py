    ...
    def relation_transform(self, pool_src, pool_dest, obj_model, res_id,
        ...
        if obj:
            ...
            if res:
                ...
                if not res:
                    obj_model_str = obj_model.replace(".","_")

                    if obj_model_str == "hr_employee":
                        res_search = self.env[obj_model].search([('name', '=', names), '|', ('active','=', True), ('active', '=', False)], limit=1)

                        _logger.debug("--------------Empleado inactivo buscado------------------")
                        _logger.debug("-------------------------------- %s", res_search.name)
                        _logger.debug("-------------------------------- %s", res_search.id)
                        _logger.debug("-------------------------------- %s", res_search)

                        _logger.debug("--------------Agregando Syncro------------------")
                        _logger.debug("---------"+str(res_search.id)+"--------------------------")
                        result = res_search.id
