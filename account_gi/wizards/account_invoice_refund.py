...
class account_invoice_refund_gi(models.TransientModel):
    ...
    reason_type = fields.Many2one(
        string='Tipo de razon',
        comodel_name='account.res_reasons_types',
        required=True,
        store=True,
    )
    ...
    def compute_refund(self, mode='refund'):
        ...
        for form in self:
            ...
            for inv in inv_obj.browse(context.get('active_ids')):
                ...
                reason = form.reason_type
                refund = inv.refund(form.date_invoice, date, description, inv.journal_id.id, comment, reason)
